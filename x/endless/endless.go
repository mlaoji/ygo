...
/*
fork from https://github.com/fvbock/endless
*/
...
*/
func NewServer(addr, network string, handler http.Handler, otherServer OtherServer) (srv *endlessServer) { // {{{
...
*/
func ListenAndServe(addr string, handler http.Handler, rtimeout, wtimeout time.Duration, mhbytes int) error { // {{{
	server := NewServer(addr, "", handler, nil)

	server.Server.ReadTimeout = rtimeout
	server.Server.WriteTimeout = wtimeout
	server.Server.MaxHeaderBytes = mhbytes

	return server.ListenAndServe()
} // }}}

func ListenAndServeOther(addr, network string, otherServer OtherServer) error { // {{{
	server := NewServer(addr, network, nil, otherServer)
	return server.ListenAndServe()
} // }}}
...
func (srv *endlessServer) Serve() (err error) { // {{{
	...
	if nil != srv.otherServer {
		err = srv.otherServer.Serve(srv.EndlessListener)
	} else {
...
func (srv *endlessServer) getListener(laddr string) (l net.Listener, err error) { // {{{
	if srv.isChild {
		...
		if srv.network != "" {
			...
			addr, err = net.ResolveTCPAddr(srv.network, laddr)
			if err != nil {
				err = fmt.Errorf("net.ResolveTCPAddr error: %v", err)
				return
			}
...
func (el *endlessListener) Accept() (c net.Conn, err error) { // {{{
	if el.server.network != "" {
		tc, err := el.Listener.Accept()
		if err != nil {
			return nil, err
		}
	...
	} else if lis, ok := el.Listener.(*net.TCPListener); ok {
		tc, err := lis.AcceptTCP()
		if err != nil {
			return nil, err
		}
